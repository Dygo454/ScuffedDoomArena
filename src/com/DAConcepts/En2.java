...
public class En2 {
    ...
    private static float attackingDurr = 0.3f;
    ...
    public En2(Game g) {
        game = g;
        ss = g.getEnemySpriteSheet2();
        start = new Vector2(0,0);
    }

    public En2(float x, float y, Game g) {
        xPos = x;
        yPos = y;
        game = g;
        ss = g.getEnemySpriteSheet2();
    ...
    public En2(float x, float y, float zR, Game g) {
        xPos = x;
        yPos = y;
        zRot = zR;
        game = g;
    ...
    private void attack() {
        if (isDead()) return;
        int off = (int) (Math.random()*19)-9;
        for (int i = 0; i < 9; i++) {
            float mult = (float) Math.random();
            float xAng = ((i-4)/4f + mult)*2+off;
            Vector2 transformForward = game.player.transformForward(zRot+xAng-90);
            Object hit = Physics.raycast(transformForward,new Vector2(xPos,yPos),this);
            if (hit == null) {
                return;
            }
            if (hit.getClass() == Player.class) {
                game.player.damage((float) Math.random()*5f);
            }
            if (hit.getClass() == En1.class) {
                En1 e = (En1) hit;
                e.damage((float) Math.random()*5f);
            }
            if (hit.getClass() == En2.class) {
                En2 e = (En2) hit;
                e.damage((float) Math.random()*5f);
            }
        }
